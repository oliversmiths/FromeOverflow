...
- **`docs/lib/format.js`** — shared duration maths and formatting (`spillMs`,
  ...
  `overflow_context` fields as a `.pop-context` term/value grid, plus up to two
  more rows from `fmtAnnualReturn` — "EA `<year>`" (spill count + duration) and
  "EA avg" (the long-term average, its own row rather than a trailing aside on
  the year, since it's a different, multi-year figure) — the Environment
  Agency's own regulator-verified numbers, deliberately the *other* figure
  from everything else on the page (ours is a live-tracked floor; theirs is
  official, counted differently, a year in arrears). Rows with no value are
  skipped, so an unfetched monitor just shows the feed half, and a monitor
  with no annual-return match just skips those two rows. `dayCells`
