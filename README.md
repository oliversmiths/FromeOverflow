...
discharging, grey no data. It opens tight on the town centre; pan, zoom and pinch
out to a rectangle shaped to the Frome catchment — long north–south, reaching
...
- The open tab is remembered in the URL (`#timeline` / `#about` / `#safety` / `#sources`).
...
`docs/basemap.json` (~410 KB); the page draws it as SVG. Towns and villages show
at any zoom, suburbs / hamlets / road names once you zoom in. Re-run it whenever
...
npm run dev        # both
npm run basemap    # rebuild docs/basemap.json (the map's streets)
npm run audit-ids  # check PIN_TO_IDS against the live feed
```

First poll prints how many overflows Wessex operate region-wide and how many are
in the Frome list.
...
it down to a **fixed, hand-curated list of 38 monitor IDs** (`PIN_TO_IDS` in
`poll.js`) — every outfall on the River Frome and its tributaries within reach of
town. Edit that list by hand to add or drop one.

The list was generated by a fallback rule (kept in `poll.js` for reference):
within 7 km of the centre, or within 30 km on a watercourse named after a Frome
tributary (Mells, Nunney Brook, Rodden Brook, Whatley Brook, …). The 30 km bound
stays well clear of the *Bristol* Frome, a different river ~31 km away.

Run `npm run audit-ids` every few months — it fetches the feed, re-applies the
rule, and prints what Wessex have added or retagged so you can update the list.
Nothing about the set drifts on its own.

A monitor with no coordinates is skipped; one dropped from the list ages off the
page after a week but keeps its history in the database.
